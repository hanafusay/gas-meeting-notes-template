/**
 * Google Meet 議事録 → GitHub 自動 push GAS（Gemini API 整形あり版）
 *
 * Google Meet + Gemini Notes が自動生成した議事録 Docs を、
 * 指定の Bot カレンダーに紐づいた予定の attachments から取得し、
 * Gemini API で再整形してから GitHub に直接 push する。
 *
...
const PLACEHOLDER_API_KEY = '<PASTE_GEMINI_API_KEY>';
...
const setupScript = (): void => {
  ...
  if (!props.getProperty('CALENDAR_IDS')) {
    ...
    console.log(`CALENDAR_IDS をデフォルト値で設定しました: ${DEFAULT_CALENDAR_IDS}`);
  } else {
    console.log(`CALENDAR_IDS は既存値を使用します: ${props.getProperty('CALENDAR_IDS')}`);
  }

  if (!props.getProperty('GITHUB_TOKEN')) {
    props.setProperty('GITHUB_TOKEN', PLACEHOLDER_TOKEN);
    console.log('GITHUB_TOKEN に placeholder を設定しました。GAS UI から実際の PAT に書き換えてください。');
  }

  if (!props.getProperty('GEMINI_API_KEY')) {
    props.setProperty('GEMINI_API_KEY', PLACEHOLDER_API_KEY);
    console.log('GEMINI_API_KEY に placeholder を設定しました。GAS UI から実際の API キーに書き換えてください。');
  }
  ...
  ScriptApp.newTrigger('syncMeetingNotes').timeBased().everyMinutes(15).create();
  console.log('トリガー設定: syncMeetingNotes を15分おきに実行');

  const remaining: string[] = [];
  if (props.getProperty('GITHUB_TOKEN') === PLACEHOLDER_TOKEN) {
    remaining.push(`GITHUB_TOKEN: ${GITHUB_OWNER}/${GITHUB_REPO} への Contents Read/Write 権限を持つ GitHub Fine-grained PAT を貼り付け`);
  }
  if (props.getProperty('GEMINI_API_KEY') === PLACEHOLDER_API_KEY) {
    remaining.push('GEMINI_API_KEY: https://aistudio.google.com/apikey で発行した API キーを貼り付け');
  }

  if (remaining.length > 0) {
    console.warn(`セットアップ未完了です。GAS エディタ > プロジェクトの設定 > スクリプトプロパティ で以下を設定してください:\n- ${remaining.join('\n- ')}`);
  } else {
...
const syncMeetingNotes = (): void => {
  ...
  if (!githubToken || githubToken === PLACEHOLDER_TOKEN) {
    console.error('GITHUB_TOKEN が未設定または placeholder のままです。GAS スクリプトプロパティで実際の PAT に書き換えてください。');
    return;
  }
  ...
  console.log(`syncMeetingNotes start. CALENDAR_IDS=${JSON.stringify(calendarIds)}, range=${yesterday.toISOString()}..${now.toISOString()}`);
  ...
  console.log(`syncMeetingNotes done. processed=${processed} skipped=${skipped} errored=${errored}`);
};
...
const processCalendar = (
  ...
  try {
    ...
    console.error(`Calendar.Events.list failed for ${calendarId}: ${(e as Error).message}`);
  ...
  for (const event of events.items) {
    try {
      ...
      console.error(`Error processing event "${event.summary}": ${(e as Error).message}`);
      errored++;
    }
  }
...
const processEvent = (
  ...
  console.log(`Processing event: ${summary} (eventId=${eventId}, docId=${docId})`);
  ...
  // Gemini で議事録再生成
  const summaryText = tabs[0]?.text || '';
  const transcriptionText = tabs.slice(1).map((t) => t.text).join('\n\n');
  const eventStart = getEventStart(event);
  const minutesText = generateMinutesWithGemini_(
  ...
  const files = buildMarkdownFiles_(tabs, summary, eventStart, basePath, minutesText, eventId, event.htmlLink || '');
...
const isAlreadyProcessed = (eventId: string, githubToken: string): boolean => {
  ...
  if (status !== 200) {
    console.warn(`GitHub search API status=${status}: ${response.getContentText().substring(0, 200)}`);
    return false;
  }
...
const getProcessedCache = (): Set<string> => {
  ...
  try { return new Set<string>(JSON.parse(raw)); } catch (_e) { return new Set<string>(); }
};
...
const getAllDocumentTabs_ = (
  ...
  if (statusCode !== 200) {
    console.error(`Docs API error (${statusCode}): ${response.getContentText().substring(0, 300)}`);
    return getTabsFallback_(docId);
  }

  const doc = JSON.parse(response.getContentText());
  const tabs = doc.tabs || [];
  if (tabs.length === 0) return getTabsFallback_(docId);

  const result: Array<{ title: string; text: string; index: number }> = [];
  for (const tab of tabs) {
    const tabTitle = tab.tabProperties?.title || `タブ${(tab.tabProperties?.index || 0) + 1}`;
    const text = extractTextFromDocContent_(tab.documentTab?.body?.content || []);
    if (text.trim().length > 0) result.push({ title: tabTitle, text: text.trim(), index: tab.tabProperties?.index || 0 });
    for (const childTab of tab.childTabs || []) {
      const childTitle = childTab.tabProperties?.title || `${tabTitle}_子タブ${(childTab.tabProperties?.index || 0) + 1}`;
      const childText = extractTextFromDocContent_(childTab.documentTab?.body?.content || []);
      if (childText.trim().length > 0) result.push({ title: childTitle, text: childText.trim(), index: childTab.tabProperties?.index || 0 });
    }
  }
  return result;
};
...
const extractTextFromDocContent_ = (content: any[]): string => {
  ...
  for (const element of content) {
    if (element.paragraph) {
      ...
      const paragraphText = (element.paragraph.elements || []).map((el: any) => {
        if (el.textRun) {
      ...
      if (paragraphText.trim().length === 0) { text += '\n'; continue; }
      if (prefix) {
        if (!isFirstHeading) text += '\n---\n\n';
    ...
    if (element.table) {
      for (const row of element.table.tableRows || []) {
        ...
        const cells = (row.tableCells || []).map((cell: any) =>
          extractTextFromDocContent_(cell.content || []).trim()
        ).join(' | ');
        text += `${cells}\n`;
      }
      text += '\n';
...
const getTabsFallback_ = (
  ...
  try {
    ...
    if (bodyText.length > 0) return [{ title: 'メインコンテンツ', text: bodyText, index: 0 }];
  } catch (e) {
  ...
  if (response.getResponseCode() !== 200) throw new Error(`Drive API export failed: ${response.getResponseCode()}`);
...
const generateMinutesWithGemini_ = (
  ...
  if (!apiKey || apiKey === PLACEHOLDER_API_KEY) {
    console.warn('GEMINI_API_KEY が未設定または placeholder のままです。議事録生成をスキップします');
    return null;
  }

  const dateFormatted = Utilities.formatDate(eventDate, Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');
  const userMessage = [
    `会議タイトル: ${subject}`,
    `日時: ${dateFormatted}`,
    '',
  ...
  return callGeminiAPI_(apiKey, MINUTES_PROMPT, userMessage);
};

const callGeminiAPI_ = (
  apiKey: string,
  systemPrompt: string,
  userMessage: string,
): string | null => {
  ...
  const status = response.getResponseCode();
  if (status !== 200) {
    console.error(`Gemini API error (${status}): ${response.getContentText().substring(0, 300)}`);
    return null;
  }
  const result = JSON.parse(response.getContentText());
  const candidates = result.candidates || [];
  if (candidates.length === 0 || !candidates[0].content) return null;
  // biome-ignore lint/suspicious/noExplicitAny: Gemini API response is loosely typed
...
const buildMarkdownFiles_ = (
  ...
  if (htmlLink) lines.push(`> カレンダーURL: ${htmlLink}`);
  lines.push('');

  if (minutesText) {
    lines.push('---', '', '## 議事録（AI生成）', '', minutesText, '');
  }
...
const pushToGitHub_ = (
  ...
  for (const file of files) {
    const blobRes = fetchGitHubAPI_(`${apiBase}/git/blobs`, 'post', { content: file.content, encoding: 'utf-8' }, headers);
    treeItems.push({ path: file.path, mode: '100644', type: 'blob', sha: blobRes.sha });
  }

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    const refRes = fetchGitHubAPI_(`${apiBase}/git/ref/heads/${GITHUB_BRANCH}`, 'get', null, headers);
    const headCommitSha = refRes.object.sha;
    const commitRes = fetchGitHubAPI_(`${apiBase}/git/commits/${headCommitSha}`, 'get', null, headers);
    const treeRes = fetchGitHubAPI_(`${apiBase}/git/trees`, 'post', { base_tree: commitRes.tree.sha, tree: treeItems }, headers);

    const now = new Date().toISOString();
    const newCommitRes = fetchGitHubAPI_(`${apiBase}/git/commits`, 'post', {
      message: commitMessage,
      tree: treeRes.sha,
      parents: [headCommitSha],
      author: { name: 'GAS AutoSave', email: 'gas-autosave@users.noreply.github.com', date: now },
      committer: { name: 'GAS AutoSave', email: 'gas-autosave@users.noreply.github.com', date: now },
    }, headers);

    const refUpdateRes = fetchGitHubAPIWithStatus_(`${apiBase}/git/refs/heads/${GITHUB_BRANCH}`, 'patch', { sha: newCommitRes.sha }, headers);

    if (refUpdateRes.ok) {
      console.log(`GitHub commit: ${newCommitRes.sha.substring(0, 7)}` + (attempt > 1 ? ` (retried ${attempt})` : ''));
      return;
    }
    ...
    throw new Error(`GitHub ref update failed (${refUpdateRes.status}): ${refUpdateRes.body}`);
  }
};
...
// biome-ignore lint/suspicious/noExplicitAny: returns parsed JSON
  ...
  const options: GoogleAppsScript.URL_Fetch.URLFetchRequestOptions = { method, headers, muteHttpExceptions: true };
  if (payload) { options.contentType = 'application/json'; options.payload = JSON.stringify(payload); }
  const response = UrlFetchApp.fetch(url, options);
  const status = response.getResponseCode();
  if (status < 200 || status >= 300) throw new Error(`GitHub API ${status}: ${response.getContentText().substring(0, 200)}`);
...
const fetchGitHubAPIWithStatus_ = (
  ...
  const response = UrlFetchApp.fetch(url, options);
  const status = response.getResponseCode();
  return { ok: status >= 200 && status < 300, status, body: response.getContentText() };
};
